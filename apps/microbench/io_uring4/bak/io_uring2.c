...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < writes_per_pipe; i++) {
        ...
        // for (int j = 0; j < num_pipes * 2 - 99; j += io_uring_submit(&ring));
        ...
        //for (int ii=0; ii < num_pipes*2; ii++)
        //    io_uring_wait_cqe(&ring, cqes);
        //io_uring_wait_cqe(&ring, cqes);
        //printf("ret: %d\n",ret);
        /*
        int ret = io_uring_wait_cqe_nr(&ring, cqes, num_pipes * 2);
        if (ret < 0) {
            printf("error waiting for any completions\n");
            return 0;
        }
        */
        //io_uring_wait_cqe_nr(&ring, cqes, num_pipes * 2);
        for (int j=0; j<num_pipes; j++) {
        /*
