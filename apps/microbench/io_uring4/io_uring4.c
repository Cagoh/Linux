...
const char f_io_uring_wait_cqe_nr[] = "io_uring_wait_cqe_nr";
const char f_io_uring_wait_cqe_nr2[] = "io_uring_wait_cqe_nr2";
...
int child_process(struct io_uring *ring, struct io_uring_sqe *sqe, struct io_uring_cqe **cqes, struct io_uring *ring2, struct io_uring_sqe *sqe2, struct io_uring_cqe **cqes2) {
    ...
    for (int i = 0; i < writes_per_pipe; i++) {
        ...
        M5OP_TRACEPOINT(INST_TP_FUNC_ENTRY,&f_io_uring_wait_cqe_nr[0]);
        if (io_uring_wait_cqe_nr(ring, cqes, num_pipes) < 0) {
            printf("error waiting for any completions\n");
            return 0;
        }
        M5OP_TRACEPOINT(INST_TP_FUNC_EXIT,&f_io_uring_wait_cqe_nr[0]);
        
        M5OP_TRACEPOINT(INST_TP_FUNC_ENTRY,&f_io_uring_peek_batch_cqe[0]);
        io_uring_peek_batch_cqe(ring, cqes, MAX_BATCH);
        ...
        //io_uring_prep_read(sqe2, rand_write[j] * 2, buffer.iov_base + BUF_SIZE * rand_write[j] * 2, BUF_SIZE, 0);
    ...
    M5OP_TRACEPOINT(INST_TP_FUNC_ENTRY,&f_io_uring_cq_advance[0]);
        io_uring_cq_advance(ring, num_pipes);
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < writes_per_pipe; i++) {
        ...
        M5OP_TRACEPOINT(INST_TP_FUNC_ENTRY,&f_io_uring_wait_cqe_nr2[0]);      
        if (io_uring_wait_cqe_nr(&ring2, cqes2, num_pipes) < 0) {
            printf("i: %d\n", i);
            printf("error waiting for any completions\n");
            return 0;
        }
        M5OP_TRACEPOINT(INST_TP_FUNC_EXIT,&f_io_uring_wait_cqe_nr2[0]);
        
        M5OP_TRACEPOINT(INST_TP_FUNC_ENTRY,&f_io_uring_peek_batch_cqe2[0]);
        io_uring_peek_batch_cqe(&ring2, cqes2, MAX_BATCH);
        ...
        io_uring_cq_advance(&ring2, num_pipes);
