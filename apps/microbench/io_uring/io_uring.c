...
/*
    int count = 0;
    printf("After malloc cqes:\n");
    for (int i = 0; i < writes_per_pipe; i++)
        for (int j = 0; j < num_pipes; j++) {
            printf("cqes[%d]: %d\n",i*num_pipes+j, cqes[i*num_pipes+j]);
            count += 1;
            printf("count: %d\n",count);
        }
    //printf("count: %d\n",count);
    ...
    for (int i = 0; i < writes_per_pipe; i++) {
        ...
        while (remaining_completions) {
            ...
            for (int j = 0; j < completions; j++) {
                ...
                //printf("Now at i l: %d %d\n",i,l);
                //printf("cqes[l]: %d\n", cqes[l]);
                //printf("cqes[l]->res: %d\n\n", cqes[l]->res);
                /* Check for failire */
                if (cqes[j]->res < 0) {
                //if (cqes[i*num_pipes+l]->res < 0) {
                ...
                //if (cqes[i*num_pipes+l]->res != BUF_SIZE) {
