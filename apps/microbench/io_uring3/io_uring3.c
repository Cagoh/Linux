...
/*
void *listener_thread(void *data, struct *io_uring ring) {
...
//int child_process(struct io_uring *ring, struct io_uring_sqe *sqe, struct io_uring_cqe **cqes, struct io_uring *ring2, struct io_uring_sqe *sqe2, struct io_uring_cqe **cqes2, int efd, int efd2) {
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < num_pipes; i++) {
        ...
        //if (pipe2(&pipes[i * 2], O_CLOEXEC | O_DIRECT)) {
        if (pipe2(&pipes[i * 2], O_NONBLOCK | O_CLOEXEC)) {
    ...
    */
    ...
    //printf("Parent: buffer.iov_base = %d\n", buffer.iov_base);
    //printf("Parent: buffer.iov_len = %d\n", buffer.iov_len);
    //printf("buffer.*iov_base = %d\n", *(buffer.iov_base));
    //printf("buffer.*iov_len = %d\n", *(buffer.iov_len));
    //printf("buffer.&iov_base = %d\n", &(buffer.iov_base));
    //printf("buffer.&iov_len = %d\n", &(buffer.iov_len));
    ...
    int child_success = 0;
    //int *parent_submitted_ptr;	// to record the parents iterations number
    //int parent_submitted = 0;
    //parent_submiited_ptr = &parent_submitted;
    ...
    if (child_pid == 0) {  
        ...
        //child_success = child_process(&ring, sqe, cqes, &ring2, sqe2, cqes2, efd, efd2);
    ...
    //printf("Parent process: &parent_submitted = %d\n", &parent_submitted);
